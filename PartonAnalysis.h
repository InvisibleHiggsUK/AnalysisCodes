...
public :
   ...
   PartonAnalysis(TTree *tree=0);
   virtual ~PartonAnalysis();
   virtual Int_t    Cut();
...
#ifdef PartonAnalysis_cxx
PartonAnalysis::PartonAnalysis(TTree *tree) : fChain(0) 
{
...
PartonAnalysis::~PartonAnalysis()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t PartonAnalysis::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t PartonAnalysis::LoadTree(Long64_t entry)
{
...
void PartonAnalysis::Init(TTree *tree)
{
...
Bool_t PartonAnalysis::Notify()
{
   return kTRUE;
}

void PartonAnalysis::Show(Long64_t entry)
{
   if (!fChain) return;
   fChain->Show(entry);
}

Int_t PartonAnalysis::Cut()
{
   return 1;
}
#endif // #ifdef PartonAnalysis_cxx
